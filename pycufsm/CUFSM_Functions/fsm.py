...
def strip(
    ...
    if sum(gbt_con['glob']) + sum(gbt_con['dist']) \
        ...
        [main_nodes, meta_elements, node_props, n_main_nodes, \
            ...
            = pycufsm.CUFSM_Functions.cfsm.base_properties(nodes=nodes_base, elements=elements)
        [r_x, r_z, r_yd, r_ys, r_ud] = pycufsm.CUFSM_Functions.cfsm.mode_constr(
            nodes=nodes_base,
            elements=elements,
        ...
        [d_y, n_global_modes] = pycufsm.CUFSM_Functions.cfsm.y_dofs(
            nodes=nodes_base,
            elements=elements,
            main_nodes=main_nodes,
    ...
    for i, length in enumerate(lengths):
        ...
        if cfsm_analysis == 1:
            ...
            b_v_l = pycufsm.CUFSM_Functions.cfsm.base_column(
        ...
        if bc_flag == 1:
            ...
            # BWS June 5 2006)
            r_user = pycufsm.CUFSM_Functions.cfsm.constr_user(nodes=nodes, constraints=constraints, m_a=m_a)
        ...
        if cfsm_analysis == 1:
            ...
            b_v = pycufsm.CUFSM_Functions.cfsm.base_update(
                gbt_con=gbt_con,
                b_v_l=b_v_l,
                length=length,
                m_a=m_a,
            ...
            b_v = pycufsm.CUFSM_Functions.cfsm.mode_select(
                b_v=b_v,
                n_global_modes=n_global_modes,
                n_dist_modes=n_dist_modes,
                n_local_modes=n_local_modes,
...
def m_recommend(
    ...
    _, _, _, _, _, _, n_dist_modes, n_local_modes, _ = pycufsm.CUFSM_Functions.cfsm.base_properties(
